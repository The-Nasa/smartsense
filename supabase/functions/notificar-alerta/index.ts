...
serve(async (req) => {
  ...
  try {
    ...
    // --- CONECTAR CON SUPABASE PARA LEER LAS PREFERENCIAS DEL USUARIO ---
    const supabaseUrl = Deno.env.get('SUPABASE_URL') ?? ""
    const supabaseServiceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? ""
    const supabase = createClient(supabaseUrl, supabaseServiceKey)

    // Valores por defecto
    let resendToEmail = Deno.env.get('RESEND_TO_EMAIL') || 'tapullimasernam@gmail.com'
    let twilioTo = Deno.env.get('TWILIO_TO_NUMBER') || ''
    let notifEmail = true
    let notifSms = false

    try {
      console.log("Fetching user profile preferences (id = 1) from Supabase...")
      const { data: user, error: userError } = await supabase
        .from('usuarios')
        .select('email, telefono, notif_email, notif_sms')
        .eq('id', 1)
        .single()

      if (!userError && user) {
        if (user.email) {
          resendToEmail = user.email
        }
        if (user.telefono) {
          twilioTo = user.telefono
        }
        notifEmail = user.notif_email !== false
        notifSms = user.notif_sms === true
        console.log(`Preferences loaded: Email=${resendToEmail} (${notifEmail ? 'Habilitado' : 'Deshabilitado'}), SMS=${twilioTo} (${notifSms ? 'Habilitado' : 'Deshabilitado'})`)
      } else {
        console.warn("Could not load user from DB, using fallback credentials:", userError)
      }
    } catch (dbErr) {
      console.error("Database query failed, using defaults:", dbErr)
    }

    // --- PARTE 1: ENVÍO DE EMAIL CON RESEND ---
    const resendApiKey = Deno.env.get('RESEND_API_KEY')
    let emailSent = false
    let emailError = null

    if (notifEmail && resendApiKey) {
      console.log(`Sending email warning using Resend to ${resendToEmail}...`)
      try {
        const emailResponse = await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            from: 'SmartSense IoT <onboarding@resend.dev>',
            to: [resendToEmail],
        ...
        if (!emailResponse.ok) {
          const resendErr = await emailResponse.text()
          throw new Error(resendErr)
        }
      ...
      console.log(`Skipping email: Send flag is ${notifEmail} and API key presence is ${!!resendApiKey}`)
    }
    ...
    // Solo enviamos SMS si el usuario habilitó notificaciones SMS y las credenciales están configuradas
    if (notifSms && twilioSid && twilioAuthToken && twilioFrom && twilioTo) {
      console.log(`Sending SMS alert via Twilio to ${twilioTo}...`)
      try {
      ...
      console.log(`Skipping Twilio SMS: Send flag is ${notifSms} or Twilio credentials/phone number are not fully configured in Secrets.`)
    }
